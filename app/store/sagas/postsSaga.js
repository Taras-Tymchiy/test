...
import { type LoadPostsAction, type StartPullingPostsAction } from '../actions/PostActions';
...
function* startPulling({interval, queryParams}: StartPullingPostsAction) {
  while (true) {
...
export default function* postsSaga() {
  let action: StartPullingPostsAction;
  while (action = yield take('POSTS_START_PULLING')) {
    const task = yield fork(startPulling, action);
    yield take('POSTS_STOP_PULLING');
    yield cancel(task);
  }
}
